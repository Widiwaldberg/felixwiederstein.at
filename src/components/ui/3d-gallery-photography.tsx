...
// R3F's auto-size measures container via getBoundingClientRect, which is
// post-transform when an ancestor has a CSS scale (e.g. our hero-frame GSAP
// shrink). That makes the drawing buffer too small. Force it to match the
// viewport so the canvas is always full-resolution regardless of parent scale.
function ForceViewportSize() {
	...
	useEffect(() => {
		const apply = () => {
			...
			gl.setSize(w, h, false); // false = don't touch canvas CSS (we use 100%)
	...
	const handleWheel = useCallback(
		(event: WheelEvent) => {
			if (rotationDone.current) {
				// After first rotation: scrolling down OR scrolling up while page can
				// still scroll up → let Lenis/browser handle page scroll.
				// Only intercept when at very top + scrolling up → keep gallery
				// spinning infinitely upward (default infinite-roulette behaviour).
...
export default function InfiniteGallery({
	...
	style,
	fadeSettings = {
		fadeIn: { start: 0.15, end: 0.28 },
		fadeOut: { start: 0.4, end: 0.43 },
	},
	blurSettings = {
		blurIn: { start: 0.15, end: 0.28 },
